...
```{r}
texto_vetor <- xpathSApply(pagina, "//article[@class = 'news']/p" , xmlValue)
texto <- c()
...
for (link in links_datafolha){
  ...
  titulo <- nullToNA(xpathSApply(pagina, "//h1[@class = 'main_color main_title']", xmlValue))
  datahora <- nullToNA(xpathSApply(pagina, "//time", xmlValue))
  pesquisa <- nullToNA(xpathSApply(pagina, "//p[@class = 'stamp download']/a" , xmlGetAttr, name = "href"))
  ...
  dados <- rbind(dados, data.frame(titulo, datahora, pesquisa, texto, check.rows = T))
}
```
